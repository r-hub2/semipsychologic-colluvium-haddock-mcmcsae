  ...
  if (mc[["type"]] == "block" || !mc[["in_block"]]) {
    ...
    if (mc[["e"]]$control$expanded.cMVN.sampler) {
      ...
      if (is.null(add.outer.R)) {
        ...
        if (add.outer.R && !is.null(R)) {
          ...
          cols <- i_apply(mc[["mcs"]], function(x) if (is.null(x[["R"]])) 0L else ncol(x[["R"]]))
          cols <- cols[cols != 0L]
          if (length(cols) > 1L) {
      ...
      if (!add.outer.R || is.null(R)) {
        ...
        tryCatch(
          ...
          error = function(e) {
            ...
            if (is.null(R)) {
              if (mc[["type"]] == "block") {
                noninf <- b_apply(mc[["mcs"]], function(x) isFALSE(x$informative.prior))
                if (any(noninf))
                  warn("error in setting up a multivariate normal sampler, possibly due to ",
                       ...
                       "of model component(s) '", paste0(names(noninf)[noninf], collapse="', '"),
      ...
      if (is.null(mc$MVNsampler)) {  # 2nd attempt or add.outer.R
        ...
        tryCatch(
          ...
          error = function(e) {
            if (mc[["type"]] == "block") {
              noninf <- b_apply(mc[["mcs"]], function(x)
    ...
    if (mc[["type"]] == "gen" && mc[["unit_Q"]]) {  # unit QA, scalar Qv (with unit Q0), and no constraints
      if (is_unit_diag(XX)) {
        ...
        if (is_unit_ddi(XX.expanded)) XX.expanded <- expand_unit_ddi(XX)
